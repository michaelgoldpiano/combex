...
const out = {
  ...
  objEmpty: () => combo() as Combinator<never>,
};

describe('some', () => {
  describe('arr', () => {
    it('[_, _]', () => success(() => some(p1.arr()), out.arr));
    it('[[_], [_]]', () => success(() => some(p1.arrArr()), out.arrArr));
    it('[combo(_), combo(_)]', () => success(() => some(p1.arrCombo()), out.arrCombo));
    it('[_, lit(_, combo(_))]', () => success(() => some(p1.arrLiteral()), out.arrLiteral));
    it('[]', () => success(() => some(p1.arrEmpty()), out.arrEmpty));
  });

  describe('obj', () => {
    it('obj', () => success(() => some(p1.obj()), out.obj));
    it('{{_}, {_}}', () => success(() => some(p1.objObj()), out.objObj));
    it('{combo(_), combo(_)}', () => success(() => some(p1.objCombo()), out.objCombo));
    it('{_, lit(_, combo(_))}', () => success(() => some(p1.objLiteral()), out.objLiteral));
    it('{}', () => success(() => some(p1.objEmpty()), out.objEmpty));
  });
});